...
for prof in range(PROF_START, PROF_END):
    text = []
    r = requests.get("https://www.work.ua/resumes/%s/" % str(prof))
    text = r.text.split("\n")
    ...
    if text[33].find("<title>Сторiнку не знайдено. Помилка 404 — Work.ua</title>") == -1:
        try:
            ...
            print("Some error with picture line with %s: %s" % (str(prof), if_picture))
            continue
        if if_pcture.find('<img width="300" height="400" src="') != -1:
            image_url = text[224].replace("\n", "").replace('                            <img width="300" height="400" src="', "").replace('" alt="">', "")
            ...
            if file_c > 35000:
                ...
                os.mkdir(jpg_picsfolder)
                file_c = 0
            rep_str = text[5].replace('    <meta name="Description" content="', "").replace('">', "").replace("&nbsp", " ")
            date_resume = text[222].replace('                            <span class="text-muted">', "").split('</span>')[0].replace("&nbsp;", " ")
            rep_str = " ".join([rep_str, date_resume, "(вік станом на %s)" % DATEOFPROC])
            try:
...
report.close()
print("Found %d resumes with a picture; %d without picture were skipped" % (file_total, bad_f))
