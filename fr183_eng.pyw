...
# Global vars - Start
appcurver = "Faces Recognition 1.83 by Bohdan SOKRUT and Python 3.8, dlib, face_recognition"
Dir_List = {}
    ...
    for entry in entries:
        ...
        if (entry.name.split(".")[-1].lower() in ["bmp", "gif", "jpg", "jpeg", "png"]) and entry.is_file():
            ...
            if fl_MultyTh:  # trying to use MultyThreads
                ...
                # maybe cnn - more accurate and use GPU/CUDA,
...
def dir_load_wantedimg(parwnd):  # Loading and encoding wanted people
    ...
    if answ != None:  # setting tolerance for facecomp
        ...
        if answ == 2:
            mod = "cnn"
    directory = sel_dir(
        parwnd, "Choose a folder with the pistures of wanted individual(s)", Dir_List, False, False)
    if directory in [".", "", None]:
        del(wantedEncodings)
        del(wantedNames)
        return
...
def optim():
    ...
    if tk.messagebox.askyesno("Attention", "Small face encodings data files to be consolidated for faster face search. Proceed?"):
        ...
        if fcnt == 0:
            tk.messagebox.showwarning(
                "Attention!", "No small data files found.")
            del(Encodings)
            del(Names)
            return
        ...
        try:
            ...
            tk.messagebox.showwarning(
                "Attention!", "Can't write data file %s. Mannualy move data files back from %s to %s." % (fn, backupdir, knwdbdir))
            del(Encodings)
            del(Names)
            return
...
def pic_search(parwnd):
    ...
    wrksx.write(0, 0, "Report created " + str(datetime.now().strftime("%Y-%m-%d %H.%M.%S")) + " by %s" % appcurver, cell_bold)
    wrksx.write(1, 0, "Accuracy: " + str(tol), cell_bold)
    wrksx.write(3, 0, "Wanted individual picture file", cell_bold)
    wrksx.write(3, 1, "Wanted individual's picture thumbnail", cell_bold)
    wrksx.write(3, 2, "Reference individual's picture thumbnail", cell_bold)
    wrksx.write(3, 3, "Reference individual's picture file", cell_bold)
    ...
    xlcnt = 4 # starting row at xlsx=worksheet for the results
    for r in allfound:
