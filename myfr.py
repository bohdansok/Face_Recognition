    ...
    def save(self):
        ...
        if bool(self.dicfilename) and self.mode == "save":
            try:
                ...
                tk.messagebox.showwarning(
                    "Увага!", "Не можу створити файл бази даних %s" % self.dicfilename)
                return False
    ...
    def save_as(self, filename):
        ...
        if bool(filename):
            try:
                ...
                tk.messagebox.showwarning(
                    "Увага!", "Не можу створити файл бази даних %s" % filename)
                return False
            pickle.dump(self.fd, f, protocol=pickle.HIGHEST_PROTOCOL)
            f.close()
            return True   
...
def get_params(fl_dir_comment=True):
    #Defaults
    mod = "hog"
    nous = 1
    njits = 1
...
def splitvid(root):
    ...
    fl_MultyTh = False
    frfl = face_recognition.face_locations
    vidfile = tk.filedialog.askopenfilename(master=root, title="Обеpіть файл відео для обробки",
