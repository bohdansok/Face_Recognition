...
def dir_load_allimg(parwnd):
    ...
    if not os.path.exists(knwdbdir):
        try:
            ...
            tk.messagebox.showwarning("Увага!", "Не можу створити робочу теку %s" % knwdbdir)
            return
    ...
    for entry in allimgf:
        if os.path.isfile(entry):
            ...
            if fl_MultyTh:
                ...
                image = frlif(entry.path)
                boxes = frfl(image, model= mod) # may be cnn - more accu and use GPU/CUDA,
            if len(boxes) > 0: #more then 1 face in 1 pic - various face encodds' with the same oic file
                if fl_MultyTh:
                    encies = executor.submit(frfe, image, boxes).result()
                else:
   ...
   ### making local copies of global funcs
    ...
    if not os.path.exists(wntdbdir):
        try:
            ...
            tk.messagebox.showwarning("Увага!", "Не можу створити робочу теку %s" % wntdbdir)
            del(frlif)
            del(frfl)
            del(frfe)
            return
    ...
    if tk.messagebox.askyesno("Увага!", "Дрібні файли даних кодувань облич буде консолідовано для прискорення пошуку. Продовжити?"):
        ...
        if not os.path.exists(backupdir):
            try:
                ...
                tk.messagebox.showwarning("Увага!", "Не можу створити теку резервних копій %s" % backupdir)
                return
        bigdic = {}
        fcnt = 0
        ...
        for entry in entries:
            if entry.name.lower().endswith(".pkl") and entry.is_file() and os.path.getsize(entry.path) > pkl_min_sz and os.path.getsize(entry.path) < pkl_max_sz:
                ...
                fcnt += 1
        if fcnt == 0:
            tk.messagebox.showwarning("Увага!", "Немає дрібних файлів для оптимізації!")
            del(Encodings)
            del(Names)
            return
        ...
        try:
            ...
            tk.messagebox.showwarning("Увага!","Не можу створити файл бази даних %s. Перенесіть файли з даних з теки %s назад до теки %s" % (fn, backupdir, knwdbdir))
            del(Encodings)
            del(Names)
            return
...
def pic_search(parwnd):
    ...
    wntdbdir = os.path.join(os.path.join(os.getcwd(), "_Wanted"))
    if not os.path.exists(wntdbdir):
        tk.messagebox.showwarning("Увага!", "Не можу знайтии робочу теку %s" % wntdbdir)
        return
                ...
                if True in match:
                    ...
                    for i in matchedIdxs:
                        ...
                        try:
                                ...
                                t1t = t1 + "_192.jpg" # ескізу для gif-формату не буде
                                im.save(t1t)
                                tmpwlist.append(t1t)
                        ...
                        try:
                                im = Image.open(t2)
                                im.thumbnail((192, 192))
                                t2t = t2 + "_192.jpg" # ескізу для gif-формату не буде
                                im.save(t2t)
